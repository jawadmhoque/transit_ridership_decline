...
def prepare_dataframe(df):
    ...
    col_name.extend(["VRM_ADJ_log_FAC", "FARE_per_UPT_cleaned_2018_log_FAC",
                     "YEARS_SINCE_TNC_BUS_FAC", 'YEARS_SINCE_TNC_RAIL_FAC'])
    ...
    for metro in tqdm(metroes):
        ...
        if int(start_year) <= (2012):
            for col in col_name:
                if col not in ["ID", "Year", "RAIL_FLAG", "CLUSTER_APTA", "UPT_ADJ"]:
                    # df_filter.loc[df_filter.ID == 103, 'FirstName'] = "Matt"
                    # df_filter['Year'].dt.year >= int(2012)
...
def prepare_chart(df, start_year, end_year, file_name):
    ...
    for mode in modes:
        ...
        subplot_labels = ['Vehicle Revenue Miles',
                          ...
                          '% of Population + Employment in Transit Supportive Density',
        ...
        for chartcol, subplotlable in zip(chartcols, subplot_labels):
            ...
            # mask = df_fltr_mode['Year'].dt.year >= int(2012)
            # df_b4_2012 = df_fltr_mode[~mask]
            # df_aft_2012 = df_fltr_mode[mask]
            ...
            ax[row][col].set_xlabel(xlabel="Year", fontsize=12)
            ax[row][col].tick_params(labelsize=13)
            ax[row][col].legend(loc='best', fontsize=9)
            ax[row][col].set_title(str(subplotlable), fontsize=12)
            try:
