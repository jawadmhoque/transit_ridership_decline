...
def prepare_dataframe(df, i=None):
    ...
    df_metro = df.copy().loc[:, col_name]

    # replace the null values with 0
    df_metro = check_nullvalues(df_metro, col_name)

    # merge fields "BUS" and "RAIL"
    df_metro["VRM_ADJ_log_FAC"] = df_metro["VRM_ADJ_BUS_log_FAC"] + df_metro["VRM_ADJ_RAIL_log_FAC"]
    df_metro["FARE_per_UPT_cleaned_2018_log_FAC"] = df_metro["FARE_per_UPT_cleaned_2018_BUS_log_FAC"] + \
                                                    df_metro["FARE_per_UPT_cleaned_2018_RAIL_log_FAC"]
    ...
    df_metro['YEARS_SINCE_TNC'] = df_metro["YEARS_SINCE_TNC_BUS_HINY_FAC"] + df_metro[
        "YEARS_SINCE_TNC_BUS_MIDLOW_FAC"] + \
                                  df_metro["YEARS_SINCE_TNC_RAIL_HINY_FAC"] + df_metro["YEARS_SINCE_TNC_RAIL_MID_FAC"]
    ...
    #             'FARE_per_UPT_cleaned_2018_RAIL_log_FAC', 'YEARS_SINCE_TNC_BUS_HINY_FAC','YEARS_SINCE_TNC_BUS_MIDLOW_FAC',
    #             'YEARS_SINCE_TNC_RAIL_HINY_FAC','YEARS_SINCE_TNC_RAIL_MID_FAC','MAINTENANCE_WMATA_FAC',
    #             'RESTRUCTURE_FAC','New_Reporter_FAC']
    # col_name.remove(col_list)
    ...
    df_metro_summary['New_Competing_Modes'] = df_metro_summary["YEARS_SINCE_TNC"] + \
                                              df_metro_summary["BIKE_SHARE_FAC"] + \
                                              df_metro_summary["scooter_flag_FAC"]
    # summarised colcharts
    sum_col_name = get_filtered_summary_columns()
    df_metro_summary = df_metro_summary.copy().loc[:, sum_col_name]

    # get the unique metro names
    metroes = df_metro['ID'].unique()

    # Delete if the script output folder is already existing
    # Check file in Path = Factors and Ridership Data\code
    current_dir = pathlib.Path(__file__).parent.absolute()
    get_dir_path = current_dir.parents[0] / 'Script Outputs' / 'Est11_Outputs' / 'Set2'
...
def metro_area(metro, df, col_name):
    file_name = str(metro)
    ...
    get_dir_path = current_dir.parents[0] / 'Script Outputs' / 'Est11_Outputs' / 'Set 2'/'Metro_Area_CSVs'
...
def summary_metro_area(metro, df, sum_col_name):
    file_name = str(metro)
    df_filter = df.copy()[df.ID == str(metro)]
    start_year = get_startyear(df_filter)
    end_year = get_endyear(df_filter)
    ...
    current_dir = pathlib.Path(__file__).parent.absolute()
    # Change the directory - Script Outputs\Est11_Outputs\Metro_Area_CSVs
    get_dir_path = current_dir.parents[0] / 'Script Outputs' / 'Est11_Outputs'/ 'Set 2'/ 'Metro_Area_Summary_CSVs'
    # Check if the above directory path exists or not, if not then create it
    pathlib.Path(get_dir_path).mkdir(parents=True, exist_ok=True)
    os.chdir(str(get_dir_path))
    # export the metro file as CSV
    ...
    for mode in modes:
        ...
        for chartcol, subplotlable in zip(chartcols, subplot_labels):
            ...
            try:
                ...
                ax[row][col].set_ylim([min_val*0.6, max_val * 1.25])
            except ValueError:
                pass
